...
def convert_collection_to_df(customer_data):
    ...
    ## Convert dob from str to datetime.date
    #
    ...
    #
    # customer_df['dob'] = customer_df['dob'].where(pd.notnull(customer_df['dob']), None)
    # customer_df['dob'] = customer_df['dob'].fillna(pd.Timestamp('1900-01-01').date())
...
def send_dataframe_to_clickhouse(data_interval_start=None, data_interval_end=None):
    ...
    if len(rows) == 0:
        ...
        last_time_update = rows[0]['last_update']
    ...
    clickhouse_client.insert(
        table='retailDataWareHouse.dim_customer',
        data=rows,
