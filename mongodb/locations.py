...
def convert_collection_to_df(location_data):
    ...
    if not(location_df.empty) :
        ...
        location_df = location_df[['_id' , 'ward_id' , 'name', 'city_name', 'update_date', 'createdAt']]
        location_df.rename({'_id':'ward_id', 'ward_id':'ward_key', 'name':'ward_name' ,'createdAt': 'created_at', 'update_date': 'updated_at'}, axis=1, inplace=True)

        location_df['ward_id'] = location_df['ward_id'].astype(str)
        location_df['ward_key'] = location_df['ward_key'].astype(str)
        location_df['ward_name'] = location_df['ward_name'].astype(str)
